...
gh pr view 123 --json body,title,labels,files | npx monobrain@latest swarm create-from-pr

# Auto-spawn agents based on PR labels
gh pr view 123 --json labels | npx monobrain@latest swarm auto-spawn

# Create swarm with PR context
gh pr view 123 --json body,labels,author,assignees | \
  npx monobrain@latest swarm init --from-pr-data
```
      ...
      - name: Handle Swarm Command
        run: |
          if [[ "${{ github.event.comment.body }}" == /swarm* ]]; then
            npx monobrain@latest github handle-comment \
...
npx monobrain@latest github pr-topology --pr 123
```
...
npx monobrain@latest github pr-init 123 \
  --auto-agents \
  --pr-data "$PR_INFO" \
  --diff "$PR_DIFF" \
  --analyze-impact
...
PROGRESS=$(npx monobrain@latest github pr-progress 123 --format markdown)
...
# Run swarm review
REVIEW_RESULTS=$(npx monobrain@latest github pr-review 123 \
...
npx monobrain@latest github multi-pr \
  --prs "123,124,125" \
  --strategy "parallel" \
  --share-memory
```
...
npx monobrain@latest github pr-deps 123 \
  --spawn-agents \
  --resolve-conflicts
```
...
```bash
# Auto-fix PR issues
npx monobrain@latest github pr-fix 123 \
...
SWARM_STATUS=$(npx monobrain@latest github pr-status 123)
...
createServer((req, res) => {
  if (req.url === "/github-webhook") {
    ...
    if (event.action === "opened" && event.pull_request) {
      execSync(`npx monobrain@latest github pr-init ${event.pull_request.number}`);
    }
...
npx monobrain@latest github pr-init 456 \
...
npx monobrain@latest github pr-init 789 \
  --topology mesh \
...
npx monobrain@latest github pr-init 321 \
  --topology ring \
...
npx monobrain@latest github pr-report 123 \
...
npx monobrain@latest github export-metrics \
  --pr 123 \
  --to-insights
```
...
mcp__monobrain__agent_spawn { type: "tester", name: "Test Engineer" }
