...
tools: mcp**monobrain**swarm_init, mcp**monobrain**agent_spawn, mcp**monobrain**task_orchestrate, Bash, Read, Write, TodoWrite
color: blue
type: development
capabilities:
...
npx monobrain github review-init \
  --pr 123 \
  --pr-data "$PR_DATA" \
  --diff "$PR_DIFF" \
...
# Run security review
SECURITY_RESULTS=$(npx monobrain github review-security \
  --pr 123 \
  --files "$CHANGED_FILES" \
...
npx monobrain github review-performance \
  --pr 123 \
...
```bash
# Architecture review
npx monobrain github review-architecture \
  --pr 123 \
...
npx monobrain github review-context \
  --pr 123 \
  --load-related-prs \
  --analyze-impact \
...
npx monobrain github review-learn \
...
npx monobrain github review-batch \
  --prs "123,124,125" \
  --check-consistency \
  --verify-integration \
  --combined-impact
    ...
    steps:
      ...
      - name: Run Review Swarm
        run: |
          ...
          # Run swarm review
          REVIEW_OUTPUT=$(npx monobrain github review-all \
            --pr $PR_NUM \
            --pr-data "$PR_DATA" \
...
COMMENTS=$(npx monobrain github review-comment \
  --pr 123 \
  --diff "$PR_DIFF" \
  --files "$PR_FILES" \
  --style "constructive" \
...
npx monobrain github review-comments \
  --pr 123 \
...
npx monobrain github quality-gates \
  --define '{
...
npx monobrain github review-metrics \
  --period 30d \
...
npx monobrain github review-train \
  --learn-patterns \
  --adapt-to-style \
  --improve-accuracy
```
...
npx monobrain github review-orchestrate \
  --strategy "risk-based" \
...
npx monobrain github review-dashboard \
  --real-time \
...
npx monobrain github review-report \
  --format "markdown" \
