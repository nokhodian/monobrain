...
// Export for use in parse() calls: schema.parse(data, { errorMap: securityErrorMap })
// NOT applied globally to avoid mutating shared Zod state across all consumers.
...
const PARSE_OPTS = { errorMap: securityErrorMap };
...
export class InputValidator {
  ...
  static validate<T>(schema: z.ZodSchema<T>, input: unknown): T {
    return schema.parse(input, PARSE_OPTS);
  }
  ...
  static safeParse<T>(schema: z.ZodSchema<T>, input: unknown): z.SafeParseReturnType<unknown, T> {
    return schema.safeParse(input, PARSE_OPTS);
  }
  ...
  static validateEmail(email: string): string {
    return EmailSchema.parse(email, PARSE_OPTS);
  }
  ...
  static validatePassword(password: string): string {
    return PasswordSchema.parse(password, PARSE_OPTS);
  }
  ...
  static validateIdentifier(id: string): string {
    return IdentifierSchema.parse(id, PARSE_OPTS);
  }
  ...
  static validatePath(path: string): string {
    return PathSchema.parse(path, PARSE_OPTS);
  }
  ...
  static validateCommandArg(arg: string): string {
    return CommandArgumentSchema.parse(arg, PARSE_OPTS);
  }
  ...
  static validateLoginRequest(data: unknown): z.infer<typeof LoginRequestSchema> {
    return LoginRequestSchema.parse(data, PARSE_OPTS);
  }
  ...
  static validateCreateUser(data: unknown): z.infer<typeof CreateUserSchema> {
    return CreateUserSchema.parse(data, PARSE_OPTS);
  }
  ...
  static validateTaskInput(data: unknown): z.infer<typeof TaskInputSchema> {
    return TaskInputSchema.parse(data, PARSE_OPTS);
  }
}
