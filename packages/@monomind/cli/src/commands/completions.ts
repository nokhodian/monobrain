...
function generateBashCompletion(): string {
  return `# monobrain bash completion
# Generated by monobrain completions bash
# Add this to ~/.bashrc or ~/.bash_completion

_monobrain_completions() {
...
complete -F _monobrain_completions monobrain
complete -F _monobrain_completions npx\\ @monobrain/cli@v1alpha
`;
}

// Generate zsh completion script
function generateZshCompletion(): string {
  return `#compdef monobrain
# monobrain zsh completion
# Generated by monobrain completions zsh
# Add to ~/.zfunc/_monobrain or ~/.zsh/completions/_monobrain
...
_monobrain() {
    ...
    case $state in
        command)
            _describe -t commands 'monobrain commands' commands
            ;;
        subcommand)
            case $words[2] in
                swarm)
...
function generateFishCompletion(): string {
  return `# monobrain fish completion
# Generated by monobrain completions fish
# Save to ~/.config/fish/completions/monobrain.fish

# Disable file completion by default
complete -c monobrain -f

# Top-level commands
${TOP_LEVEL_COMMANDS.map(cmd => `complete -c monobrain -n "__fish_use_subcommand" -a "${cmd}"`).join('\n')}

# Swarm subcommands
${SWARM_SUBCOMMANDS.map(sub => `complete -c monobrain -n "__fish_seen_subcommand_from swarm" -a "${sub}"`).join('\n')}

# Agent subcommands
${AGENT_SUBCOMMANDS.map(sub => `complete -c monobrain -n "__fish_seen_subcommand_from agent" -a "${sub}"`).join('\n')}

# Task subcommands
${TASK_SUBCOMMANDS.map(sub => `complete -c monobrain -n "__fish_seen_subcommand_from task" -a "${sub}"`).join('\n')}

# Memory subcommands
${MEMORY_SUBCOMMANDS.map(sub => `complete -c monobrain -n "__fish_seen_subcommand_from memory" -a "${sub}"`).join('\n')}

# Hive-mind subcommands
${HIVE_MIND_SUBCOMMANDS.map(sub => `complete -c monobrain -n "__fish_seen_subcommand_from hive-mind hive" -a "${sub}"`).join('\n')}

# Hooks subcommands
${HOOKS_SUBCOMMANDS.map(sub => `complete -c monobrain -n "__fish_seen_subcommand_from hooks" -a "${sub}"`).join('\n')}

# Neural subcommands
complete -c monobrain -n "__fish_seen_subcommand_from neural" -a "train status patterns predict optimize"

# Security subcommands
complete -c monobrain -n "__fish_seen_subcommand_from security" -a "scan cve threats audit secrets"

# Performance subcommands
complete -c monobrain -n "__fish_seen_subcommand_from performance" -a "benchmark profile metrics optimize bottleneck"

# Plugins subcommands
complete -c monobrain -n "__fish_seen_subcommand_from plugins" -a "list install uninstall toggle info create"

# Deployment subcommands
complete -c monobrain -n "__fish_seen_subcommand_from deployment deploy" -a "deploy status rollback history environments logs"

# Claims subcommands
complete -c monobrain -n "__fish_seen_subcommand_from claims" -a "list check grant revoke roles policies"

# Embeddings subcommands
complete -c monobrain -n "__fish_seen_subcommand_from embeddings" -a "generate search compare collections index providers"

# Daemon subcommands
complete -c monobrain -n "__fish_seen_subcommand_from daemon" -a "start stop status trigger enable"

# Providers subcommands
complete -c monobrain -n "__fish_seen_subcommand_from providers" -a "list configure test models usage"
`;
}

// Generate PowerShell completion script
function generatePowerShellCompletion(): string {
  return `# monobrain PowerShell completion
# Generated by monobrain completions powershell
# Add to $PROFILE or save to a separate file and dot-source it

$script:MonobrainCommands = @(
...
Register-ArgumentCompleter -Native -CommandName monobrain -ScriptBlock {
    ...
    if ($tokens.Count -eq 2) {
        ...
        $script:MonobrainCommands | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
...
export const completionsCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'monobrain completions bash > ~/.bash_completion.d/monobrain', description: 'Install bash completions' },
    { command: 'monobrain completions zsh > ~/.zfunc/_monobrain', description: 'Install zsh completions' },
    { command: 'monobrain completions fish > ~/.config/fish/completions/monobrain.fish', description: 'Install fish completions' },
    { command: 'monobrain completions powershell >> $PROFILE', description: 'Install PowerShell completions' }
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  monobrain completions bash > ~/.bash_completion.d/monobrain'));
    output.writeln(output.dim('  source ~/.bash_completion.d/monobrain'));
    output.writeln();
    output.writeln(output.bold('Zsh:'));
    output.writeln(output.dim('  mkdir -p ~/.zfunc'));
    output.writeln(output.dim('  monobrain completions zsh > ~/.zfunc/_monobrain'));
    output.writeln(output.dim('  # Add to ~/.zshrc: fpath=(~/.zfunc $fpath); autoload -Uz compinit && compinit'));
    output.writeln();
    output.writeln(output.bold('Fish:'));
    output.writeln(output.dim('  monobrain completions fish > ~/.config/fish/completions/monobrain.fish'));
    output.writeln();
    output.writeln(output.bold('PowerShell:'));
    output.writeln(output.dim('  monobrain completions powershell >> $PROFILE'));
