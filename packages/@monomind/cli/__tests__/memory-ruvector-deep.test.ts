...
describe('Memory Initializer', () => {
  describe('MEMORY_SCHEMA', () => {
    it('should export a non-empty schema string', async () => {
      const { MEMORY_SCHEMA } = await import('../src/memory/memory-initializer.js');
      expect(MEMORY_SCHEMA).toBeDefined();
      expect(typeof MEMORY_SCHEMA).toBe('string');
      expect(MEMORY_SCHEMA.length).toBeGreaterThan(100);
    });

    it('should contain core memory_entries table', async () => {
      const { MEMORY_SCHEMA } = await import('../src/memory/memory-initializer.js');
      expect(MEMORY_SCHEMA).toContain('CREATE TABLE IF NOT EXISTS memory_entries');
    });

    it('should contain patterns table for learning', async () => {
      const { MEMORY_SCHEMA } = await import('../src/memory/memory-initializer.js');
      expect(MEMORY_SCHEMA).toContain('CREATE TABLE IF NOT EXISTS patterns');
    });

    it('should contain trajectories table for SONA', async () => {
      const { MEMORY_SCHEMA } = await import('../src/memory/memory-initializer.js');
      expect(MEMORY_SCHEMA).toContain('CREATE TABLE IF NOT EXISTS trajectories');
    });

    it('should contain migration_state table', async () => {
      const { MEMORY_SCHEMA } = await import('../src/memory/memory-initializer.js');
      expect(MEMORY_SCHEMA).toContain('CREATE TABLE IF NOT EXISTS migration_state');
    });

    it('should contain sessions table', async () => {
      const { MEMORY_SCHEMA } = await import('../src/memory/memory-initializer.js');
      expect(MEMORY_SCHEMA).toContain('CREATE TABLE IF NOT EXISTS sessions');
    });

    it('should contain vector_indexes table for HNSW', async () => {
      const { MEMORY_SCHEMA } = await import('../src/memory/memory-initializer.js');
      expect(MEMORY_SCHEMA).toContain('CREATE TABLE IF NOT EXISTS vector_indexes');
    });

    it('should contain metadata table', async () => {
      const { MEMORY_SCHEMA } = await import('../src/memory/memory-initializer.js');
      expect(MEMORY_SCHEMA).toContain('CREATE TABLE IF NOT EXISTS metadata');
    });

    it('should set WAL journal mode pragma', async () => {
      const { MEMORY_SCHEMA } = await import('../src/memory/memory-initializer.js');
      expect(MEMORY_SCHEMA).toContain('PRAGMA journal_mode = WAL');
    });
  });
