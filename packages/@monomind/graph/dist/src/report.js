...
function isFileNode(graph, nodeId) {
    ...
    const sourceFile = graph.getNodeAttribute(nodeId, 'sourceFile') || '';
    if (!label || !sourceFile)
        return false;
    if (label === basename(sourceFile))
        return true;
...
function communityLabel(graph, memberIds) {
    const dirCounts = {};
    for (const id of memberIds) {
        const src = graph.getNodeAttribute(id, 'sourceFile') || '';
        if (!src)
            continue;
 ...
 */
export function generateReport(graph, analysis, cohesionScores, options = {}) {
    ...
    graph.forEachEdge((_, attrs) => {
        const c = attrs.confidence || 'EXTRACTED';
        confCounts[c] = (confCounts[c] ?? 0) + 1;
        if (c === 'INFERRED') {
            const cs = attrs.confidenceScore;
            if (cs !== undefined)
                infScores.push(cs);
        }
    });
    ...
    lines.push('', '## Summary');
    lines.push(`- ${graph.order} nodes · ${graph.size} edges · ${analysis.stats.communities} communities detected`);
    lines.push(`- Extraction: ${extPct}% EXTRACTED · ${infPct}% INFERRED · ${ambPct}% AMBIGUOUS${infTag}`);
    if (tokenCost) {
        lines.push(`- Token cost: ${tokenCost.input.toLocaleString()} input · ${tokenCost.output.toLocaleString()} output`);
    }
    // God nodes
    ...
    if (analysis.surprises.length > 0) {
        for (const s of analysis.surprises) {
            ...
            const confTag = conf === 'INFERRED'
                ? `INFERRED ${s.confidenceScore?.toFixed(2) ?? ''}`
                : conf;
            const semTag = s.relation === 'semantically_similar_to' ? ' [semantically similar]' : '';
            lines.push(`- \`${s.from}\` --${s.relation}--> \`${s.to}\`  [${confTag}]${semTag}`);
    ...
    if (hyperedges.length > 0) {
        ...
        for (const h of hyperedges) {
            const he = h;
            const nodeLabels = (he.nodes ?? []).join(', ');
            const conf = he.confidence ?? 'INFERRED';
            const cs2 = he.confidenceScore;
            const confTag = cs2 !== undefined ? `${conf} ${cs2.toFixed(2)}` : conf;
            lines.push(`- **${he.label ?? ''}** — ${nodeLabels} [${confTag}]`);
        }
    }
    // Communities
    ...
    for (const [cidStr, memberIds] of Object.entries(analysis.communities)) {
        ...
        const display = realNodes
            .slice(0, 8)
            .map((id) => graph.getNodeAttribute(id, 'label') || id);
    ...
    graph.forEachEdge((_, attrs, source, target) => {
        if (attrs.confidence === 'AMBIGUOUS') {
            ambiguous.push({ u: source, v: target, attrs: attrs });
        }
    });
    if (ambiguous.length > 0) {
        lines.push('', '## Ambiguous Edges - Review These');
        for (const { u, v, attrs } of ambiguous) {
            const ul = graph.getNodeAttribute(u, 'label') || u;
            const vl = graph.getNodeAttribute(v, 'label') || v;
            lines.push(`- \`${ul}\` → \`${vl}\`  [AMBIGUOUS]`);
            lines.push(`  ${attrs.sourceFile || ''} · relation: ${attrs.relation || 'unknown'}`);
        }
    }
    // Knowledge gaps
    const isolated = [];
    ...
    const thinCommunities = Object.entries(analysis.communities).filter(([, nodes]) => nodes.length < 3);
    ...
    if (gapCount > 0 || ambPct > 20) {
        ...
        if (isolated.length > 0) {
            ...
            lines.push(`- **${isolated.length} isolated node(s):** ${labels.map((l) => `\`${l}\``).join(', ')}${suffix}`);
