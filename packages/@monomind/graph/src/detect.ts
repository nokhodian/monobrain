...
function isSecuritySensitive(basename: string, relPath: string): boolean {
  return SECURITY_SENSITIVE_PATTERNS.some(
    (re) => re.test(basename) || re.test(relPath),
  );
}
...
export function collectFiles(
  ...
  function walkDir(dirPath: string): void {
    ...
    for (const entry of entries) {
      ...
      if (excludePatterns.some((pat) => relPath.includes(pat))) continue;
      ...
      if (isSecuritySensitive(entry, relPath)) continue;
 ...
 * looks healthy.
 */
export function corpusHealth(files: ClassifiedFile[]): string[] {
  ...
  if (codeFiles.length < 5) {
    warnings.push(
      `Corpus is very small (${codeFiles.length} code files). Graph may be sparse.`,
    );
  }
  ...
  if (totalSize < 50_000) {
    ...
    warnings.push(
      `Corpus total size is very small (~${kb}KB). Consider including more source files.`,
    );
  }
  ...
  if (totalSize > 50_000_000) {
    ...
    warnings.push(
      `Corpus is very large (~${mb}MB). Build may be slow. Consider using codeOnly or excludePatterns.`,
    );
  }
  ...
  if (files.length > 0 && docFiles.length / files.length > 0.8) {
    warnings.push(
      'Corpus is mostly documentation. Consider codeOnly: true for a code-focused graph.',
    );
  }
