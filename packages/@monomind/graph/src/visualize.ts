...
export function exportHTML(serialized: SerializedGraph, outputDir: string): string {
  ...
  // Community map
  const communityNodes: Record<number, number> = {};
  for (const n of vizNodes) {
    const c = (n as Record<string, unknown>)['community'] as number ?? -1;
  ...
  for (const l of vizLinks) {
    const r = String((l as Record<string, unknown>)['relation'] ?? 'ref');
  ...
  const godNodes = [...vizNodes]
    ...
    .map(n => ({
      id: n.id,
      label: (n as Record<string, unknown>)['label'] as string || n.id,
      degree: deg[n.id] ?? 0,
      community: (n as Record<string, unknown>)['community'] as number ?? -1,
    }));

  // Build vis.js nodes
  const visNodes = vizNodes.map(n => {
    const attrs = n as Record<string, unknown>;
    const cid = (attrs['community'] as number) ?? -1;
    const color = cid >= 0 ? PALETTE[cid % PALETTE.length] : '#64748b';
    const label = (attrs['label'] as string) || n.id;
    const d = deg[n.id] ?? 0;
