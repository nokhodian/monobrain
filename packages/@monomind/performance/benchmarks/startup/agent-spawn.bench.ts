...
/**
 * Spawn a single agent (V1 style - optimized)
 */
...
export async function runAgentSpawnBenchmarks(): Promise<void> {
  ...
  // Benchmark 2: Single Agent Spawn (V1 Style)
  const singleResult = await runner.run(
    'single-agent-spawn-v1',
    async () => {
      await spawnAgent('coder');
    },
    { iterations: 100 }
  );

  console.log(`Single Agent (current): ${formatTime(singleResult.mean)}`);
  const singleTarget = meetsTarget('agent-spawn', singleResult.mean);
  console.log(`  Target (<200ms): ${singleTarget.met ? 'PASS' : 'FAIL'}`);

  // Calculate single spawn speedup
  const singleSpeedup = singleV2Result.mean / singleResult.mean;
  ...
  // Benchmark 4: 5 Agents Parallel (V1 Style)
  ...
  // Benchmark 5: 15 Agents Parallel (V1 Swarm)
  ...
  // Summary
  console.log('\n--- Summary ---');
  console.log(`V2 -> Current Speedup: ${singleSpeedup.toFixed(2)}x`);
  console.log(`Sequential -> Parallel (5 agents) Speedup: ${parallelSpeedup.toFixed(2)}x`);
  console.log(`Pool vs Spawn Speedup: ${(singleResult.mean / poolGetResult.mean).toFixed(2)}x`);
