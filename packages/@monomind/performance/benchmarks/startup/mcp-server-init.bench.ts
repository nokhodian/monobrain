...
export async function runMCPInitBenchmarks(): Promise<void> {
  ...
  // Benchmark 6: Optimized MCP Init
  const optimizedInitResult = await runner.run(
    'optimized-mcp-init-v1-style',
    async () => {
  ...
  console.log(`Optimized MCP Init: ${formatTime(optimizedInitResult.mean)}`);
  const target = meetsTarget('mcp-server-init', optimizedInitResult.mean);
  console.log(`  Target (<400ms): ${target.met ? 'PASS' : 'FAIL'}`);
