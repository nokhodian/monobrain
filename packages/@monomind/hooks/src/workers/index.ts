...
/**
 * Creates a worker that calculates accurate V1 implementation progress.
 * Counts actual CLI commands, MCP tools, hooks, and packages.
 * Writes to v1-progress.json for statusline display.
 */
export function createProgressWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const pkgProgress = Math.min(100, (packages / 17) * 100); // 17 packages in v1
