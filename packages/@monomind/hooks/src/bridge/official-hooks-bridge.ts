...
/**
 * Mapping from V1 HookEvent to Official hook events
 */
export const HOOK_TO_OFFICIAL_MAP: Record<HookEvent, OfficialHookEvent | null> = {
  // Direct mappings
...
/**
 * Tool matchers for V1 events that map to PreToolUse/PostToolUse
 */
export const TOOL_MATCHERS: Partial<Record<HookEvent, string>> = {
...
/**
 * Bridge class for converting between V1 and official hooks
 */
export class OfficialHooksBridge {
  /**
   * Convert official hook input to V1 HookContext
   */
  static toHookContext(input: OfficialHookInput): HookContext {
    const event = this.officialToHookEvent(input.hook_event_name, input.tool_name);
  ...
  /**
   * Convert V1 HookResult to official hook output
   */
  ...
  /**
   * Convert official hook event to V1 HookEvent
   */
  static officialToHookEvent(officialEvent: OfficialHookEvent, toolName?: string): HookEvent {
  ...
  /**
   * Get tool matcher for a V1 event
   */
  static getToolMatcher(event: HookEvent): string | null {
    return TOOL_MATCHERS[event] ?? null;
  }

  /**
   * Check if V1 event maps to an official hook
   */
  static hasOfficialMapping(event: HookEvent): boolean {
    return HOOK_TO_OFFICIAL_MAP[event] !== null;
  }
...
/**
 * Execute a V1 handler and bridge to official output
 */
export async function executeWithBridge(
  input: OfficialHookInput,
  handler: HookHandler
): Promise<OfficialHookOutput> {
  const context = OfficialHooksBridge.toHookContext(input);
