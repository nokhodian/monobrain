...
Knowledge changes over time. The database host was `postgres-1` last week but is `postgres-2` now. An API was v2 until yesterday; now it's v1. Simple key-value storage loses this history.
...
const future = reasoner.whatWillBeTrue(
  "config",
...
const newId = store.assert("api", "version", "v1", {
  validFrom: new Date("2026-01-01").getTime(),
